...
function truncateLabel(label: string, sliceAngle: number): string {
  const maxChars =
    sliceAngle >= 90
      ...
      : sliceAngle >= 45
        ? MAX_CHARS_MEDIUM
        : sliceAngle >= FONT_THRESHOLD_ANGLE
          ? MAX_CHARS_NARROW
          : MAX_CHARS_TINY;
  ...
  ({
    ...
    const slices = useMemo(() => {
      ...
      return segments.map((segment) => {
        ...
        const x1 = (
          CENTER_X +
          RADIUS * Math.cos((Math.PI * currentAngle) / 180)
        ).toFixed(2);
        const y1 = (
          CENTER_Y +
          RADIUS * Math.sin((Math.PI * currentAngle) / 180)
        ).toFixed(2);
        ...
        >
          <svg
            ...
            style={{
              ...
              height: '100%',
              display: 'block',
              filter: 'drop-shadow(0 0 20px rgba(37, 123, 244, 0.3))',
            }}
          >
            <defs>
              <filter
            ...
            {slices.map((slice) =>
              slice.showLabel ? (
                <text
                  ...
                  fontSize={
                    slice.sliceAngle >= FONT_THRESHOLD_ANGLE
                      ? FONT_SIZE_NORMAL
                      : FONT_SIZE_SMALL
                  }
                  fontWeight="600"
